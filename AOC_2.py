...
puzzle_input = getListFromFile("AOC_2_data.txt", "\n")
...
#
# It seems like the submarine can take a series of commands like forward 1, do
#
# wn 2, or up 3:
#
...
horizontal, depth, answer = 0, 0, 0

for directions in puzzle_input:
    instruction = get_instructions(directions)
    path, value = instruction['name'], instruction['value']

    if path == "forward":
        horizontal += value
    else:
        depth = depth - value if path == "up" else depth + value
...
horizontal, depth, aim, answer = 0, 0, 0, 0
...
for directions in puzzle_input:
    ...
    if path == "forward":
        ...
        aim = aim - value if path == "up" else aim + value
