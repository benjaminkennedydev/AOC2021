...
# Puzzle Input, Functions

puzzle_input = getListFromFile("AOC_3_data.txt", "\n")


def recurse(arr, index, isMin):
    position = 0
    m, n, r = [], [], []
    ...
    if position < 0:
        r = m if not isMin else n
    else:
        r = n if not isMin else m

    return r if len(r) == 1 else recurse(r, index + 1, isMin)
...
for i in range(0, len(puzzle_input[0])):
    for line in puzzle_input:
        pos = pos - 1 if line[i] == "0" else pos + 1

    if pos < 0:
        m.append("0"), n.append("1")
    else:
        m.append("1"), n.append("0")

    pos = 0

answer = int("".join(m), 2) * int("".join(n), 2)
print(answer)
...
# considered.  To find CO2 scrubber rating, determine the least common value
# (0 or 1) in the current bit position, and keep only numbers with that bit
# in that position. If 0 and 1 are equally common, keep values with a 0 in
...
# each). So, to find the oxygen generator rating, keep the number
...
oxygen = recurse(puzzle_input, 0, False)
co2 = recurse(puzzle_input, 0, True)
